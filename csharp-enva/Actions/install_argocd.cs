...
public class InstallArgoCDAction : BaseAction, IAction
{
    ...
    public bool Execute()
    {
        ...
        try
        {
            ...
            Logger.GetLogger("install_argocd").Printf("Installing ArgoCD using kubectl apply...");
            string installCmd = "export PATH=/usr/local/bin:$PATH && export KUBECONFIG=/etc/rancher/k3s/k3s.yaml && kubectl apply -n argocd -f https://raw.githubusercontent.com/argoproj/argo-cd/stable/manifests/install.yaml";
            timeout2 = 300;
            int maxRetries = 3;
